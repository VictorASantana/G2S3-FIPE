...
def future_prices():
  ...
  if future_price_search: 
    ...
    if "user_info" in st.session_state and st.session_state["user_info"] != None: 
      if "user_id" not in st.session_state["user_info"]:
        user_id = None
      else:
    ...
    if len(formatted_data) == 0:
      st.error("O carro selecionado não tem dados suficientes")
    else:     
